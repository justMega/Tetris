 ...
 * Important to note that this uses MemScope() as an allocator when kVector4 is created. However, if needed you can pass your own allocator via the fourth patameter - allocator
 * @return Vector4
 */
...
/**
 * Calculate linear interpolation between two Vector4s
 */
 ...
 */
inline fun Vector4.nLerp(v4: Vector4, amount: Float): Vector4 {
    return QuaternionNlerp(this.readValue(), v4.readValue(), amount).getPointer(MemScope()).pointed
}

/**
 * Calculates spherical linear interpolation between two quaternions
 */
inline fun Vector4.sLerp(v4: Vector4, amount: Float): Vector4 {
    return QuaternionSlerp(this.readValue(), v4.readValue(), amount).getPointer(MemScope()).pointed
}

/**
 * Get the rotation angle and axis for a given quaternion
 */
inline fun Vector4.toAxisAngle(outAxis: CPointer<Vector3>, outAngle: CPointer<FloatVar>) {
 ...
 */
inline fun Vector4.toEuler(): Vector3 {
    return QuaternionToEuler(this.readValue()).getPointer(MemScope()).pointed
}
 ...
 * NOTE: Angles are returned in a Vector3 struct in radians
 */
inline fun Vector4.transform(mat: Matrix): Vector4 {
    return QuaternionTransform(this.readValue(), mat.readValue()).getPointer(MemScope()).pointed
}
...
inline fun Vector4.equalsTo(v: Vector4): Boolean {
    val result = QuaternionEquals(this.readValue(), v.readValue())
...
/**
 * Add two vectors to both x, y, z and w of the vector.
 */
...
/**
 * Set value of a Quaternion with another provided value.
